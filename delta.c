...
float angle_one = 20.0f;
float angle_two = 40.0f;
float angle_three = 60.0f;
...
void makeArmCylindersAt(float arm_angles, float center_radius, float angle, float arm_cyl_radius, float arm_cyl_length, float cyl_width, float cyl_radius)
{
  // a 'rotate and make two cylinder arms' function
  glGetFloatv(GL_CURRENT_COLOR, colors);
  glColor3f(colors[0]*0.8, colors[1]*0.8, colors[2]*0.8);
  glPushMatrix();
  glRotatef(angle,0,1,0); // now facing towards the center from the desired angle 
  glTranslatef(0,0-cyl_radius,center_radius); // moves to the middle of the two arms (middle of the capsule)
  glRotatef(arm_angles,1,0,0); // rotates to the trajectory of the arms
  glTranslatef((0-(cyl_width/2))-cyl_radius/2,0,0);
  gluCylinder(quadCylinder, arm_cyl_radius, arm_cyl_radius, arm_cyl_length, 32, 16);
  glTranslatef(cyl_width+cyl_radius,0,0);
  gluCylinder(quadCylinder, arm_cyl_radius, arm_cyl_radius, arm_cyl_length, 32, 16);
  glPopMatrix();
}
...
void makeHorizontalCapsuleAt(float height_position, float center_radius, float angle, float cyl_radius, float cyl_width)
{
  ...
  glRotatef(90,0,1,0);
  glTranslatef(0,0,0-(cyl_width/2));
  gluSphere(quadSphere, cyl_radius, 32, 16);
...
glPushMatrix();
  ...
  glEnd();
    ...
    glRasterPos3f(top_v_2[0][0],top_v_2[0][1]+0.5,top_v_2[0][2]);    printString("Servo One");
    glRasterPos3f(top_v_2[1][0],top_v_2[1][1]+0.5,top_v_2[1][2]);    printString("Servo Two");
    glRasterPos3f(top_v_2[2][0],top_v_2[2][1]+0.5,top_v_2[2][2]);    printString("Servo Three");
    
    glColor3f(0.3, 0.3, 1.0);
    glRasterPos3f(25,6,-5);    printString("OpenGL Delta Robot Simulator");
    glColor3f(0.5, 0.5, 1.0);
    glRasterPos3f(25,5,-5);    printString("(c) 2012 TimKrins");
    
    
    memset(sprinter, 0, sizeof(sprinter));
    sprintf(sprinter, "Angle One: %.1f", angle_one);
    
    glColor3f(.9f, .1f, .1f);
    glRasterPos3f(-20,-6,-7);  printString(sprinter);
    
    memset(sprinter, 0, sizeof(sprinter));
    sprintf(sprinter, "Angle Two: %.1f", angle_two);
    
    glColor3f(.1f, .9f, .1f);
    glRasterPos3f(-20,-8,-6);  printString(sprinter);
    
    memset(sprinter, 0, sizeof(sprinter));
    sprintf(sprinter, "Angle Three: %.1f", angle_three);
    
    glColor3f(.1f, .1f, .9f);
    glRasterPos3f(-20,-10,-5); printString(sprinter);
    glPopMatrix();
  
  float currentangle = 60.00;
  
  glColor3f(.9f, .1f, .1f);
  makeHorizontalCapsuleAt(0-(TOP_PLAT_THICK/2),TOP_PLAT_DIST, currentangle,(TOP_PLAT_THICK/1.9),TOP_CYL_WIDTH);
  makeArmCylindersAt( angle_one, TOP_PLAT_DIST, currentangle, (TOP_PLAT_THICK/5),TOP_ARM_LENGTH, TOP_CYL_WIDTH, (TOP_PLAT_THICK/1.9));

  currentangle = 180.00;
  
  glColor3f(.1f, .9f, .1f);
  makeHorizontalCapsuleAt(0-(TOP_PLAT_THICK/2),TOP_PLAT_DIST, currentangle,(TOP_PLAT_THICK/1.9),TOP_CYL_WIDTH); 
  makeArmCylindersAt( angle_two, TOP_PLAT_DIST, currentangle, (TOP_PLAT_THICK/5),TOP_ARM_LENGTH, TOP_CYL_WIDTH, (TOP_PLAT_THICK/1.9));

  currentangle = 300.00;
  
  glColor3f(.1f, .1f, .9f);
  makeHorizontalCapsuleAt(0-(TOP_PLAT_THICK/2),TOP_PLAT_DIST, currentangle,(TOP_PLAT_THICK/1.9),TOP_CYL_WIDTH); 
  makeArmCylindersAt( angle_three, TOP_PLAT_DIST, currentangle, (TOP_PLAT_THICK/5),TOP_ARM_LENGTH, TOP_CYL_WIDTH, (TOP_PLAT_THICK/1.9));
...
void graphics_draw(void) {
    glLoadIdentity();
   	glTranslatef(0, 0, (((glfwGetMouseWheel()*glfwGetMouseWheel())/5)*(glfwGetMouseWheel()*0.1))-70);  // move view back a bit 
    glfwGetMousePos(&xMouse,&yMouse);
    gluLookAt(
    (double)(xMouse-(window_width/2))/20, ((double)(yMouse-(window_height/2))/20)-10, 12, // look from
		0, -5-10, 0, // look here
